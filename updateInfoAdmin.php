...
<?php
    ...
    $query->bind_result($userId, $userKey, $userFName, $userLName, $userEmail);
        ...
        while ($query->fetch()) {
            ...
            <form style="all: unset;" action="updateInfoAdminConfig.php?userId=<?=$userId?>&accessKey=<?=$accessKey?>&emailAddr=<?=$emailAddr?>" method="post">
            ...
            <td>
            <input type="text" id="firstName" placeholder="<?=$userFName ?>" value="<?=$userFName ?>" pattern="^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z-']+$" name="firstName" required>
            </td>

            <td>
            <input type="text" id="lastName" placeholder="<?=$userLName ?>" value="<?=$userLName ?>" pattern="^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z-']+$" name="lastName" required>
            </td>

            <td>
            <input type="email" id="emailAddress" placeholder="<?=$userEmail ?>" value="<?=$userEmail ?>" name="emailAddress" required> </td>
            ...
            <td> <form style="all: unset;" action="updatePass.php?userId=<?=$userId?>&emailAddr=<?=$emailAddr?>" method="post">
                <button type="submit" name="userId" value="<?= $userId ?>" >Update Password</button> </form> </td>
